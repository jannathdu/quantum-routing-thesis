...
This thesis investigates temporal-state-aware compact routing with coherence-budget management in quantum-native entanglement overlay networks. The study extends the compact-routing framework by introducing explicit e-link age, effective fidelity, remaining coherence budget, and regeneration cost into routing and overlay-maintenance decisions.
...
A compact-routing protocol may identify a structurally valid path through the artificial overlay, but the selected e-links may not remain usable until the route is completed. This can occur when stored entanglement has already accumulated significant age, when the remaining coherence time is insufficient, or when regeneration delay is ignored during route selection.
...
Existing quantum-native compact-routing architectures provide scalable routing tables, artificial entanglement overlays, and bounded entangling-stretch guarantees. However, their routing decisions are primarily based on structural connectivity and generalized entangling cost, without explicitly representing the temporal condition of each stored artificial e-link.
...
\begin{enumerate}
    ...
    \item To extend the existing static entangling-cost model into a time-dependent routing metric.
    ...
    \item To design a coherence-budget-aware routing algorithm for Partial-Anchor and Full-Anchor entanglement overlays.
    ...
    \item To compare the proposed method against static compact routing, fidelity-only routing, age-only routing, fixed-threshold routing, and basic regeneration heuristics.
...
\begin{enumerate}
    ...
    \item How should regeneration resources be prioritized among local e-neighbor links, anchor links, and tracked long-range links?
...
\begin{enumerate}
    ...
    \item A temporal entangling-stretch formulation that evaluates compact-routing quality under heterogeneous age and decoherence conditions.
