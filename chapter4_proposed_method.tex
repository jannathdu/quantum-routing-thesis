...
This objective is embedded in the proposed time-dependent entangling-cost metric instead of being optimized separately. Therefore, high-fidelity and temporally stable routes receive lower routing costs during route selection.
...
\subsection{Temporal Classification and Link Availability}
...
\subsection{Full-Anchor Candidate Construction}
...
The estimated delay depends on the route because candidates may differ in swapping operations, purification requirements, and queueing conditions.
...
An e-link is retained if discarding it would violate an overlay-connectivity requirement.
...
If no route is directly feasible, candidates with a bounded number of regenerable critical links are considered.
...
:\;&
\widehat{D}_{s,d}^{\mathrm{repair}}(R,t)
\leq D_{\max}^{\mathrm{repair}},
\\
&
C_{s,d}^{\mathrm{repair}}(R,t)
\leq C_{\max}^{\mathrm{repair}}
\bigr\}.
\end{aligned}
\end{equation}
...
Generating candidate routes from existing compact routing structures avoids a complete graph search for each request and reduces the route-construction cost.
...
With bounded $K_{\max}$ and $H_{\max}$, the processing complexity scales approximately linearly with the number of active requests.
